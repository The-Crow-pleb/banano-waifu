...
module.exports = {
    ...
    run: async(client, message, args) => {
        ...
        try {
            ...
            const userReg = await userSchema.findOne({_id: UserId})
            if(!userReg._id) {
                ...
                let options = {method: "post",body: JSON.stringify({ "action": "account_balance", "account": `${userReg.account}`})}
                let fetched = await fetch(process.env.url, options)
                let accountFetch = await fetched.json()
                let balance;
                if(accountFetch.balance.length === 28) {
                    return message.reply(`Less than 1 banano\nhttps://creeper.banano.cc/explorer/account/${userReg.account}`)
                }
